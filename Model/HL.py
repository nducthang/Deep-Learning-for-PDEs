...
class PDENetLight(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1st layer
        Z1 = torch.tanh(torch.mm(self.Uz1, x) +
                        torch.mm(self.Wz1, S1) + self.bz1)
        G1 = torch.tanh(torch.mm(self.Ug1, x) +
                        torch.mm(self.Wg1, S1) + self.bg1)
        R1 = torch.tanh(torch.mm(self.Ur1, x) +
                        torch.mm(self.Wr1, S1) + self.br1)
        # * is element - wise multiplication
        H1 = torch.tanh(torch.mm(self.Uh1, x) +
                        torch.mm(self.Wh1, S1 * R1) + self.bh1)

        S2 = (Ones - G1) * H1 + Z1 * S1

        # 2nd layer
        Z2 = torch.tanh(torch.mm(self.Uz2, x) +
                        torch.mm(self.Wz2, S2) + self.bz2)
        G2 = torch.tanh(torch.mm(self.Ug2, x) +
                        torch.mm(self.Wg2, S2) + self.bg2)
        R2 = torch.tanh(torch.mm(self.Ur2, x) +
                        torch.mm(self.Wr2, S2) + self.br2)
        H2 = torch.tanh(torch.mm(self.Uh2, x) +
                        torch.mm(self.Wh2, S2 * R2) + self.bh2)
